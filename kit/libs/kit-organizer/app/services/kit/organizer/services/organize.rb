module Kit::Organizer::Services
  module Organize
    ...
    # An `operation` needs to be a callable, but it can be resolved from other format (look at `to_callable`)
    ...
    def self.sanitize_errors(result:)
      ...
      if ctx.is_a?(String)
        ...
        ctx = { errors: ctx.map { @1.is_a?(String) ? { detail: el } : @1 } }
      end
    ...
    def self.generate_callable_ctx(callable:, ctx:)
      ...
      if by_type[:keyrest]
        ...
        ctx.slice(*(by_type[:keyreq].map { @1[1] } + by_type[:key].map { @1[1] }))
      else
        {}
      end
    end
