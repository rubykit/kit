...
module Kit::Api::JsonApi::Services::Resolvers::ActiveRecord
    ...
    ->(query_node:) do
      ...
      if values.size > 0
        if field_name.is_a?(Symbol) || field_name.is_a?(String)
          Kit::Api::JsonApi::Services::Resolvers::ActiveRecord.condition_to_many(column_name: field_name, values: values)
        else
          Kit::Api::JsonApi::Services::Resolvers::ActiveRecord.condition_to_many_polymorphic(**field_name, values: values)
        end
      else
        nil
      end
...
=end
  ...
  def self.data_resolver(query_node:, model:, assemble_sql_query: nil)
    _, ctx = Kit::Api::JsonApi::Services::Resolvers::Sql.sql_query(
      ar_model:           model,
  ...
  def self.generate_data_resolver(model:, assemble_sql_query: nil)
    data_resolver = ->(query_node:) do
      Kit::Api::JsonApi::Services::Resolvers::ActiveRecord.data_resolver({
