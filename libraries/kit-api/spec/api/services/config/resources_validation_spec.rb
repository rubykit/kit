...
describe Kit::Api::Services::Config do
  ...
  context 'with an invalid config' do
    ...
    context 'with a relationship missing its type' do
      ...
      it 'fails' do
        ...
        expect(ctx[:errors][0][:detail]).to eq 'Kit::Api - Error: missing resource for relationship `author.books`'
      end
    end
    ...
    context 'with a relationship referencing an unregistered type' do
      ...
      it 'fails' do
        status, ctx = subject
        expect(status).to eq :error
        expect(ctx[:errors][0][:detail]).to eq 'Kit::Api - Error: unregistered resource `book` for relationship `author.books`'
      end
    end
