...
describe 'web|users|oauth|callback', type: :feature do
  ...
  before do
    ...
    expect(user_other.user_oauth_identities.count).to eq 0
  ...
  context 'with incompatible provider data email' do
    ...
    it 'redirect with a notification' do
      ...
      i18n_params = {
        provider:       provider,
        provider_email: user_other.email,
      }

      flash_text = I18n.t('kit.auth.notifications.oauth.errors.users_conflict', **i18n_params)
